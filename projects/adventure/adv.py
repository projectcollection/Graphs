...
# def get_deadend(explored_map, room, visited = 0):
# 	if len(room.getExits()) == 0:
...
# get nearest room with ? unknown exit
def get_nearest_room(explored_map, starting_room):
    visited = set()
    q = Queue()
    q.put(starting_room)
    while not q.empty():
        new_room = q.get()
        print(new_room)
        if len([ext for ext in explored_map[new_room].items() if ext == '?']) > 0:
            print('cool')

        if new_room not in visited:
            visited.add(new_room)
            for other_room in explored_map[new_room]:
                q.put(other_room)
...
while not deadend:
    for dir, ext in explored_map[current_room].items():
        ...
        if ext == '?':
            ...
            print('value', current_room)
            get_nearest_room(explored_map, current_room)
            deadend = True
